...
- 知识库管理接口已补齐 `PUT /knowledge-base/{id}`，用于更新知识库名称与描述。
- 前端知识库列表页已接入编辑弹窗，不再是占位提示。
- 当前知识库创建、编辑、删除都具备对应的前后端调用链路。

## 2026-04-18 增量说明：聊天工具层批量文档操作

- `DocumentService` 新增按标题匹配文档、批量设置分类、批量删除文档的服务能力。
- `ChatToolService` 已接入这些能力，可直接执行“把这些文档移动到某分类”“批量删除这些文档”一类动作。
- 当前批量动作仍基于标题匹配，属于 V1 可用实现，后续可继续演进到更结构化的 tool registry / schema 执行模型。

## 2026-04-18 增量说明：Retrieval Debug 与向量读取兼容

- `VectorStoreService` 现在兼容从 Redis 读取 `embedding` 时的多种反序列化结果，避免 JSON 序列化后的 Base64 字符串导致类型转换异常。
- `RetrievalDebug.vue` 已升级为独立的检索调试工作台，负责展示原始查询、改写查询、最终采用查询和命中结果对比。
